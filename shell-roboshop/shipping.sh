...
N="\e[0m"
SCRIPT_DIR=$PWD
MYSQL_HOST=mysql.amaravathi.today
...
if [ $? -ne 0 ]; then
    ...
    mysql -h $MYSQL_HOST -uroot -pNaveen124 < /app/db/schema.sql &>>$LOGS_FILE
    mysql -h $MYSQL_HOST -uroot -pNaveen124 < /app/db/app-user.sql &>>$LOGS_FILE
    mysql -h $MYSQL_HOST -uroot -pNaveen124 < /app/db/master-data.sql &>>$LOGS_FILE
